...
void RadioSetup()
{
  ...
  d_SerialPrintGroup(radio.isChipConnected() ? "Radio initialized (UNO)" : " Radio Uninitialized! Restart");
...
void ParseBoxes(char *resp)
{
  ...
  while (token != NULL && active_box_count < MAX_BOXES)
  {
    ...
    if (len >= 4) // Like, the input must atleast be something like "A0,1;", which is 4 [5]characters
    {
      ...
      if (comma != NULL)
      {
        *comma = '\0';
        ...
        else
        {
          ...
          active_boxes[active_box_count].address = CalcAddress(active_boxes[active_box_count].index);

          if (active_boxes[active_box_count].p_id != value) // Only update vals when it's actually changed
          {
            ...
            Packet config;
            PackPacket(config, PacketType::PKT_CONFIG, 9, FoodCondition::UNKNOWN, active_boxes[active_box_count].p_id);
...
void Poller()
{
  ...
  Packet req;
  PackPacket(req, PacketType::PKT_REQUEST, my_seq, FoodCondition::UNKNOWN, active_boxes[current_box].p_id);
